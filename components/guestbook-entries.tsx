...
export function GuestbookEntries() {
  const { data: entries, error, isLoading, mutate } = useSWR<GuestbookEntry[]>("guestbook", fetcher)
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">Failed to load entries. Please try again later.</p>
      </div>
    )
  }
  ...
  if (!entries || entries.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground text-lg">No entries yet. Be the first to sign our guestbook!</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-foreground">Recent Entries ({entries.length})</h2>
